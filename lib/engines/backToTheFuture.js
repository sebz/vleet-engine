...
B2TFEngine.prototype.connectAirVantage = function(system, mqttClient) {
    var self = this;
    ...
    return new Bromise(function(resolve, reject) {
        ...
        mqttClient.on("connect", function() {
            ...
            resolve([system, mqttClient]);
        });
        
    });
};

B2TFEngine.prototype.publishSimulatedData = function(info) {
    var system = info[0];
    var mqttClient = info[1];
    return new Bromise(function(resolve, reject) {
        
        var mqttId = system.gateway.serialNumber;
        mqttClient.publish(mqttId + "/messages/json", JSON.stringify(system.data), function() {
            console.log("[MQTT] ", system.name, " - values sent:", system.data);
            resolve(info);
        });
    });
};

B2TFEngine.prototype.acknowledgeOperations = function(info) {
    var system = info[0];
    var mqttClient = info[1];
    return new Bromise(function(resolve, reject) {
        var mqttId = system.gateway.serialNumber;
        mqttClient.subscribe(mqttId + "/tasks/json");
        ...
        mqttClient.on('message', function (topic, message) {
            ...
            var status = chance.pick(["OK", "OK", "OK", "OK", "OK", "OK", "OK", "OK", "KO"]); // 10% KO
            var dataAck = [{}];
            dataAck[0].uid = msg[0].uid;
            dataAck[0].status = status;
            if(status == "KO") 
                dataAck[0].message =  "ERROR 415";
            
            mqttClient.publish(mqttId + "/acks/json", JSON.stringify(dataAck), function() {
                console.log("[MQTT] ", system.name, " - ack sent: ", dataAck);
            });
        });
        resolve(info);
    });
...
B2TFEngine.prototype.close = function(info) {
    var mqttClient = info[1];
    return new Bromise(function(resolve, reject) {
        mqttClient.end();
        resolve(info);
...
B2TFEngine.prototype.sendData = function(system) {
    ...
    return new Bromise(function(resolve, reject) {
        ...
        self.connectAirVantage(system, mqttClient).then(self.publishSimulatedData).
                                //then(self.acknowledgeOperations).
                                then(self.close);
        //finally(close)
    });
};
        ...
        _.times(this.getDaysInPast(), function(day) {
            ...
            _.times(self.getValuesPerDay(), function(nthValue) {
                ...
                for (var dataDescriptor in self.getGenerationData()) {
                    ...
                    data[timestamp][dataDescriptor] = self.generateValue(dataDescriptor, system);
                }
            });
        });
    } catch (e) {
