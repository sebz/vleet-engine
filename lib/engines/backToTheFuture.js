    ...
    return new Bromise(function(resolve, reject) {
        ...
        mqttClient.on("connect", function() {
            ...
            mqttClient.publish(mqttId + "/messages/json", JSON.stringify(data), function() {
                mqttClient.end();
                console.log("[MQTT]", system.name, "- values sent");
                resolve();
            });
        });
    });
...
B2TFEngine.prototype.generateData = function(system) {
    ...
    _.times(this.getDaysInPast(), function(day) {
        ...
        _.forEach(self.getApplicationData(), function(dataDescriptor) {
            console.log("generate value for data:", dataDescriptor);
            try {
                data[timestamp][dataDescriptor.id] = self.generateValue(dataDescriptor);
            } catch (e) {
...
B2TFEngine.prototype.generateValue = function(dataDescriptor) {
    var generationConfig = this.getDataGenerationConfig(dataDescriptor.id);
    var generator = this.dataGenerators[dataDescriptor.id];
    if (!generator) {
        generator = this.getGenerator(generationConfig);
        this.dataGenerators[dataDescriptor.id] = generator;
    }
...
B2TFEngine.prototype.getApplicationData = function() {
    return this.simulation.application.data;
};
