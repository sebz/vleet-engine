...
*/
DailyUsageEngine.prototype.generateUsages = function(day) {
    var columns = {
    ...
    _.each(this.subscriptions, function(subscription) {
        ...
        var timestamp = moment().subtract(day, "days").format("x");
        ...
        for (var i = chance.integer({
                min: 0,
                max: 1
            }); i >= 0; i--) {
            stringifier.write([iccid, "SMS_SENT", timestamp, chance.phone({
                country: 'fr'
            })]);
        }
        for (var j = chance.integer({
                min: 0,
                max: 5
            }); j >= 0; j--) {
            stringifier.write([iccid, "SMS_RECEIVED", timestamp, chance.phone({
                country: 'fr'
            })]);
        }
...
DailyUsageEngine.prototype.getDaysInPast = function() {
    return this.simulator.simulation.generation.dailyUsage.nbDaysInPast;
};
...
DailyUsageEngine.prototype.start = function(systems) {
    ...
    const days = this.getDaysInPast();
    for(var d = days; d >= 0; d--) {
        this.generateUsages(d);
    }
};
