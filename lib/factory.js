    ...
    if (hasApplication()) {
        ...
        return getApplication(system)
            .then(function(app) {
                ...
                if (simulation.fleet.template.subscription) {
                    system.subscription = {
...
function getApplication(system) {
    if (system.type) {
        return getApplicationReference(system.type);
    } else {
        var chance = require("chance").Chance();
        return Bromise.resolve(chance.pick(simulation.fleet.template.applications.uid));
    }
}

function getApplicationReference(typeName) {
    return airvantage.queryApplications({
        isReference: true,
        type: typeName
    }).then(function(app) {
...
function getSystemType() {
    var chance = require("chance").Chance();
    return chance.pick(simulation.fleet.template.systemType) + "";
}

function getOperator() {
    var chance = require("chance").Chance();
    return chance.pick(simulation.fleet.template.subscription.operators) + "";
}
